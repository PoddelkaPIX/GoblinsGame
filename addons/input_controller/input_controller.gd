...
enum InputDevice {KEYBOARD_AND_MOUSE, JOYPAD}

var _input_device: InputDevice:
	set(value):
		if _input_device == value: return
		_input_device = value
		input_device_changed.emit(_input_device)
...
func _input(event: InputEvent) -> void:
	if event is InputEventJoypadButton or event is InputEventJoypadMotion:
		_input_device = InputDevice.JOYPAD
	else:
		_input_device = InputDevice.KEYBOARD_AND_MOUSE
